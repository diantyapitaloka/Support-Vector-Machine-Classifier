...
- Support vectors are the specific data points located closest to the decision boundary, often represented to the "outliers" of each group. These points are critical because they are the only observations that actually determine the position and orientation of the separating line.
...
- When data can be separated by a simple straight line, as in your chicken and snake example, it is known as Linear SVM. However, a this also for more complex data that overlaps, SVM can be used to a "kernel trick" to track map the data into higher dimensions where a clear boundary can be found.
- Variety of Specialized Kernels: Beyond basic straight lines, SVM can deploy specialized functions to fit the different data shapes. For instance, a Radial Basis Function such as (RBF) kernel can draw a circular boundary around a clump of pythons surrounded by chickens, while a Polynomial kernel creates flexible, curved partitions.
...
- Extreme Sensitivity to Feature Scaling: SVM relies directly on calculating physical distance between data points. If python length is measured in millimeters (thousands) while chicken weight is in kilograms (single digits), the model will unfairly ignore weight unless all features are normalized to the same scale before training.
- Scaling Beyond Two Classes (Multi-Class Classification): While natively designed for binary choices (chicken vs. python), SVM easily scales if you add more animals to the farm (like hawks or foxes). It uses strategies like One-vs-Rest (chicken vs. non-chicken) or One-vs-One (pairing every animal against every other animal) to build multi-cage boundaries.
- Guaranteed Global Optimum (Convex Optimization): Unlike neural networks—which can get to stuck in "good enough" local solutions during training—SVM training involves solving a convex quadratic programming problem. This guarantees that the calculated boundary is mathematically the single best possible solution for the given data.
